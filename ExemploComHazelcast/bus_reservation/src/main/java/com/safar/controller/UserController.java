...
public class UserController {
    ...
    public  ResponseEntity<User> deleteUser(@PathVariable("userId") Long userId, @RequestParam(required = false) String key) throws UserException, AdminException {
        User deletedUser= userService.deleteUser(userId, key);
        return new ResponseEntity<User>(deletedUser, HttpStatus.OK);
    }

    @GetMapping("/admin/user/{userId}")
    public  ResponseEntity<User> viewUserById(@PathVariable("userId") Long userId, @RequestParam(required = false) String key) throws UserException, AdminException {
